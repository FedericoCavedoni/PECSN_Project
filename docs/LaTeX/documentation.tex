...
\begin{itemize}
    \item Hosts base stations and users physically and memorizes their parameters in order to retreive some of them during the simulation via parent pointers.
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item It also records statistics every time a packet is \textit{dropped} or \textit{forwarded} and about \textit{queue length} and \textit{response time}.
\end{itemize}
