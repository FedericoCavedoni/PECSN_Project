    ...
    for module, metrics in vectors.items():
        if key in metrics:
            ...
            if subsample_rate is not None and subsample_rate > 0:
                ...
                n_keep = int(max(1, np.floor(len(mean_times) * keep_fraction)))
                indices = np.linspace(0, len(mean_times) - 1, n_keep, dtype=int)
...
if __name__ == "__main__":
    file_name = "N250_I01_S1e3" # Nome file json
    JSON_INPUT_FILE = f"data/{file_name}.json" #Path del file json
    
    SUBSAMPLE_RATE = 10 #None
    QUEUE_Y_LIMITS = None #(0, 10)
    RESPONSE_Y_LIMITS = None #(0, 100)
    X_LIMIT = None #(100, 900)
